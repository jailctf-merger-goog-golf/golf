...
var Transaction = class _Transaction {
  constructor(startState, changes, selection2, effects, annotations2, scrollIntoView3) {
    ...
    if (!annotations2.some((a) => a.type == _Transaction.time))
      this.annotations = annotations2.concat(_Transaction.time.of(Date.now()));
  }
  /**
  @internal
  */
  static create(startState, changes, selection2, effects, annotations2, scrollIntoView3) {
    return new _Transaction(startState, changes, selection2, effects, annotations2, scrollIntoView3);
  }
  /**
...
function resolveTransactionInner(state, spec, docSize) {
  let sel = spec.selection, annotations2 = asArray(spec.annotations);
  if (spec.userEvent)
    annotations2 = annotations2.concat(Transaction.userEvent.of(spec.userEvent));
  return {
...
var DOMObserver = class {
  ...
  readSelectionRange() {
    ...
    if (!selection2)
      return false;
    let range = browser.safari && view2.root.nodeType == 11 && view2.root.activeElement == this.dom && safariSelectionRangeHack(this.view, selection2) || selection2;
...
function safariSelectionRangeHack(view2, selection2) {
  if (selection2.getComposedRanges) {
    let range = selection2.getComposedRanges(view2.root)[0];
    if (range)
  ...
  markers(view2) {
    ...
    for (let r of state.selection.ranges) {
      ...
      if (r.empty || conf.drawRangeCursor) {
        ...
        for (let piece of RectangleMarker.forRange(view2, className, cursor2))
          cursors.push(piece);
      }
    }
    return cursors;
...
// node_modules/codemirror/node_modules/@codemirror/commands/dist/index.js
...
function cursorByPage(view2, forward) {
  ...
  let { state } = view2, selection2 = updateSel(state.selection, (range) => {
...
var findNext = /* @__PURE__ */ searchCommand((view2, { query }) => {
  ...
  view2.dispatch({
    selection: selection2,
    effects: [announceMatch(view2, next), config2.scrollToMatch(selection2.main, view2)],
...
var replaceNext = /* @__PURE__ */ searchCommand((view2, { query }) => {
  ...
  if (next) {
    ...
    effects.push(state.facet(searchConfigFacet).scrollToMatch(selection2.main, view2));
  }
  view2.dispatch({
    changes: changeSet,
    selection: selection2,
...
function defaultPositionInfo(view2, list, option, info, space2, tooltip) {
...
function prefixMatch2(options) {
  ...
  let source = toSet2(first) + toSet2(rest) + "*$";
  return [new RegExp("^" + source), new RegExp(source)];
}
function completeFromList2(list) {
  let options = list.map((o) => typeof o == "string" ? { label: o } : o);
  let [validFor, match] = options.every((o) => /^\w+$/.test(o.label)) ? [/\w*$/, /\w+$/] : prefixMatch2(options);
  return (context) => {
    let token = context.matchBefore(match);
    return token || context.explicit ? { from: token ? token.from : context.pos, options, validFor } : null;
  };
...
var pickedCompletion2 = /* @__PURE__ */ Annotation.define();
...
function snippet(template) {
  ...
  return (editor, completion, from, to) => {
    ...
    let spec = {
      ...
      scrollIntoView: true,
      annotations: completion ? [pickedCompletion2.of(completion), Transaction.userEvent.of("input.complete")] : void 0
    };
    if (ranges.length)
    ...
    if (ranges.some((r) => r.field > 0)) {
      ...
      if (editor.state.field(snippetState, false) === void 0)
        effects.push(StateEffect.appendConfig.of([snippetState, addSnippetKeymap, snippetPointerHandler, baseTheme6]));
    }
...
var globalCompletion = /* @__PURE__ */ ifNotIn(dontComplete, /* @__PURE__ */ completeFromList2(/* @__PURE__ */ globals.concat(snippets)));
...
var createTheme = ({ variant, settings, styles }) => {
  ...
  const extension = [theme3, syntaxHighlighting(highlightStyle)];
  return extension;
};
...
var annotations = new EditorView({
  ...
  doc: "",
  extensions: [basicSetup, python(), coolGlow, [theme2], silentlisten]
  // EditorView.lineWrapping],
});
window.theme = theme2;
window.view = view;
